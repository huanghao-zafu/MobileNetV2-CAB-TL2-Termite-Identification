...
class RandomSaltPepperNoise(object):
    ...
    def __call__(self, img: Image.Image) -> Image.Image:
        ...
        # 💡 【完美闭环点】: 通过显式多维索引，确保 RGB/三通道同时被强行污染，彻底封锁彩色混淆
...
def train_one_epoch(
    ...
    """
    升级后的单轮训练引擎：
    1. 支持 MixUp 与 CutMix 的 Batch 级别随机轮动切换（真正激活复合地狱难度）
    2. 利用数学期望融合，实现混淆增强下 100% 精确的 Accuracy 过程控制
    """
    model.train()
    running_loss = 0.0
    running_correct = 0
    ...
    for images, labels in pbar:
        ...
        if current_mode == "mixup":
            ...
            # 💡 数学闭环点 1：因为是类内混合，targets_a 永远等于 targets_b，直接累加绝对精确
            _, preds = outputs.max(1)
            running_correct += preds.eq(targets_a).sum().item()
            ...
            # 💡 数学闭环点 2：应对标准 CutMix 跨类突变，采用加权决策期望计算真实 Acc，消灭统计粉饰
            _, preds = outputs.max(1)
...
#     train_model(args)
# =====================================================================
# 修改位置：E:\imageDesigning\TermiteClassifier\train_termite_cls.py 的最末尾
# =====================================================================
# if __name__ == "__main__":
#     args = parse_args()
#
#     # 💡 1. 保持你原本的标准训练逻辑不动（如果你不跑训练，这行可以先不管它）
#     # train_model(args)
#
#     # 💡 2. 🔥 【硬核追加】：利用原生环境，直接发起鲁棒性极限施压评测！
#     print("\n" + "=" * 60)
#     print("🎬 启动原生环境·垂直领域极限鲁棒性对抗评测 (噪声比例: 8.0%)")
#     print("=" * 60)
#
#     device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
#
...
#         transforms.Resize((256, 256)),
#         transforms.CenterCrop(224),
#         RandomSaltPepperNoise(noise_ratio_range=(0.08, 0.08), p=1.0),
#         transforms.ToTensor(),
#         transforms.Normalize(imagenet_mean, imagenet_std),
#     ])
#
...
#     test_set = datasets.ImageFolder(test_dir, transform=robust_tf)
#     test_loader = DataLoader(test_set, batch_size=args.batch_size, shuffle=False, num_workers=0)
#     num_classes = len(test_set.classes)
#
#     # 🛡️ 评测模型 1：ResNet18
#     print("\n[📢 评测进行中] 正在加载并评估 ResNet18...")
#     model_res = get_model("res18", num_classes=num_classes)
#     if os.path.exists("best_res18.pth"):
#         model_res.load_state_dict(torch.load("best_res18.pth", map_location=device))
#         model_res.to(device).eval()
#         correct_res = 0
#         with torch.no_grad():
#             for imgs, lbls in test_loader:
#                 imgs, lbls = imgs.to(device), lbls.to(device)
#                 _, preds = model_res(imgs).max(1)
#                 correct_res += preds.eq(lbls).sum().item()
#         acc_res = (correct_res / len(test_set)) * 100.0
#     else:
#         acc_res = None
#         print("❌ 未找到 best_res18.pth")
#
#     # 🛡️ 评测模型 2：Ours 完全体 (mbv2_tl2)
#     print("\n[📢 评测进行中] 正在加载并评估 Ours 完全体模型 (mbv2_tl2)...")
#     model_ours = get_model("mbv2_tl2", num_classes=num_classes)
#     if os.path.exists("best_mbv2_tl2.pth"):
#         # 💡 使用主脚本最原生的严格加载，彻底粉碎层级错位阴伤！
#         model_ours.load_state_dict(torch.load("best_mbv2_tl2.pth", map_location=device))
#         model_ours.to(device).eval()
#         correct_ours = 0
#         with torch.no_grad():
#             for imgs, lbls in test_loader:
#                 imgs, lbls = imgs.to(device), lbls.to(device)
#                 _, preds = model_ours(imgs).max(1)
#                 correct_ours += preds.eq(lbls).sum().item()
#         acc_ours = (correct_ours / len(test_set)) * 100.0
#     else:
#         acc_ours = None
#         print("❌ 未找到 best_mbv2_tl2.pth")
#
#     # ========================================================
#     # 📊 终极战报输出
#     # ========================================================
#     print("\n" + "=" * 60)
#     print("🏆 极限恶劣环境对抗评测最终战报 (主脚本原生对齐版)")
#     print("=" * 60)
#     if acc_res is not None:
#         print(f"📉 经典残差大网络 ResNet18 抗压准确率:  {acc_res:.2f}%")
#     if acc_ours is not None:
#         print(f"🔥 我们的完全体网络 (Ours-CAB-TL2) 准确率: {acc_ours:.2f}%")
#     print("=" * 60)
#     if acc_res is not None and acc_ours is not None:
#         print(f"💡 真实学术结论: Ours 模型领先大网络高达 {acc_ours - acc_res:.2f}%！")
#     print("=" * 60)
# =====================================================================
# 终极全模型对抗修改：E:\imageDesigning\TermiteClassifier\train_termite_cls.py 最末尾
# 目的：一键自动化评测所有对比模型与消融模型，生成整张论文表格数据
# =====================================================================
if __name__ == "__main__":
    # 1. 解析命令行参数
    args = parse_args()

    # 2. 保持标准训练注释（纯推理评测，不重新训练）
    # train_model(args)

    print("\n" + "=" * 70)
    print("🎬 启动原生环境·全量网络复合空域特征阻断学术评测 (8%椒盐 + 15%大面积擦除)")
    print("=" * 70)
    ...
    robust_tf = transforms.Compose([
        ...
        transforms.ToTensor(),
        # 连续区域特征断裂核心阻断
        transforms.RandomErasing(p=1.0, scale=(0.10, 0.20), ratio=(0.5, 2.0), value=0.0),
        transforms.Normalize(imagenet_mean, imagenet_std),
    ])
    ...
    # 4. 💡 【学术全量白名单】：定义所有需要参与极限抗压对比的模型字典
    # 格式： "模型简称": ("用于 factories 的模型全称", "对应的最佳权重.pth")
    EVAL_MODELS_MATRIX = {
        "MobileNetV1 (远古组)": ("mbv1", "best_mbv1.pth"),
    ...
    for display_name, (model_tag, weight_file) in EVAL_MODELS_MATRIX.items():
        if os.path.exists(weight_file):
            ...
            print(f"⚠️  未在当前目录下找到 [{weight_file}]，跳过该组评测。")
    ...
    print("=" * 70)
    print(f"{'模型名称 (Architecture Models)':<38} | {'极限抗压准确率 (Robustness Acc)'}")
    print("-" * 70)

    for name, score in results_report.items():
        if "Ours" in name:
            print(f"🔥 {name:<35} | \033[1;31m{score:.2f}%\033[0m")
        else:
            print(f"📉 {name:<35} | {score:.2f}%")

    print("=" * 70)
    print("💡 结论生成器：请直接将此面板的数据复刻进你大修论文的 Table 4 (Robustness Matrix) 中。")
    print("=" * 70)